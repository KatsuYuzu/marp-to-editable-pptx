...
3. **test fixture 63 slides**：`pptx-export.md` はすべての既知エッジケースを網羅した canonical fixture。新しいバグを発見したら必ず対応するスライドをこのファイルに追加してから修正する
...
（現在は 63 スライド。ADR-16 対応後に Slide 62・63 が追加された。）
...
### ADR-15: flex/grid コンテナの直接テキストノードが blockChildren 存在時に消失

**問題**  
`html: true` + カスタムクラスを使った目次スライド（`.agenda-item` など `display:flex` の div の中に `inline-flex` バッジ span + 直接テキストノード "○○について" という構造）を PPTX に出力すると、バッジの数字だけが表示され、隣接するアジェンダテキストが PPTX から完全に消えた。

**根本原因: `element.children` と `element.childNodes` の責務混在**

`walkElements` は `Array.from(parent.children)` でループする（`Element` ノードのみ）。  
一方、`extractTextRuns` は `Array.from(element.childNodes)` でループする（`Text` ノードを含む）。

通常の div（`display:block`/`display:flex` でも子が全て element の場合）は問題ない。  
問題はコンテナ内に **Element の子要素（→ blockChildren）と TEXT_NODE の直接テキスト** の両方が同居するケースだった。

```
<div class="agenda-item" style="display:flex">
  <span class="num" style="display:inline-flex; background:...">1</span>  ← Element → blockChild
  Agenda text  ← Text node → walkElements には見えない
</div>
```

処理フロー（修正前）：

1. `walkElements(agenda-item)` → `span.num`（inline-flex）を `else` ブランチで処理  
   → `blockChildren = walkElements(span.num)` → inline-only → `extractTextRuns(span)` → `paragraph("1")` として `blockChildren` に追加  
2. `blockChildren.length > 0` → `container { children: [paragraph("1")] }` を出力してパス終了  
3. TEXT_NODE `"Agenda text"` は `parent.children` に含まれないため **永遠に走査されない**

**なぜ既存テストが検知できなかったか**

テストケースが「コンテナ = 純粋に block-level な子要素群」か「コンテナ = inline-only なテキスト」の二択で設計されていた。  
「blockChildren を生成する子要素 AND 直接 TEXT_NODE が同一コンテナ内に共存する」ケースを検証するテストが存在しなかった。  
また、このパターンが現れるのは `html: true` + カスタム CSS クラスの自作 HTML 目次コンポーネントのみであり、通常の Marp Markdown 記法では発生しない。

**修正箇所（`dom-walker.ts` — `walkElements` の `else` ブランチ）**

`blockChildren.length > 0` のケースでコンテナを出力した後に、  
コンテナの `childNodes` を shallow に走査して **テキストノードと inline-level 要素のみ** から paragraph を追加するパスを追加した。  
block-level な子要素は `blockChildren` で処理済みのため shallow パスでは明示的にスキップし、テキスト重複を防ぐ。

**テスト追加（`dom-walker.test.ts`）**

- `text node sibling to badge span inside flex item is NOT lost` — flex item 内の直接 TEXT_NODE が失われないことを検証
- `two-column grid with badge+text items — all item texts extracted` — 2列グリッドの各セルで badge+テキストのテキストが両方抽出されることを検証

**フィクスチャ追加（`pptx-export.md` Slide 61）**  
`display:grid` ラッパー + 6 つの `display:flex` アジェンダアイテム（各アイテムに `inline-flex` バッジ span + 直接テキストノード）を含む目次スライドを追加。修正前はテキストが完全に消える・修正後は正しく表示されることを視覚比較で確認できる。

**overlap 修正（2nd commit）**  
テキストノードの抽出修正のみでは、paragraph の x 座標がコンテナ左端（バッジと同じ位置）から始まり視覚的に重なっていた。`computeLeadingOffset(blockChildren, ...)` を適用して paragraph x を先頭 blockChild の右端まで offset するよう追加修正した。

---

### ADR-K01: 類似パターンとして残存している潜在バグ（要観察）

以下は ADR-15 の根本原因調査で発見した、現時点では表面化していないが構造的に類似するリスク。  
発生条件を整理し、修正の優先度判断の根拠として記録する。

#### K01-A: `skipInlineBadges` が block-level 再帰時に引き継がれない（中リスク）

**コード**：`extractTextRuns` の block 分岐（line ~225）

```typescript
if (/^(block|flex|grid|list-item|table)/.test(elStyle.display)) {
  runs.push(...extractTextRuns(el))  // ← skipInlineBadges を渡していない
}
```

inline 分岐では `extractTextRuns(el, skipInlineBadges)` と正しく引き継ぐが、block 分岐は常に `false` で再帰する。

**影響シナリオ**：  
`html: true` + 見出しや段落の中に `display:flex` な div があり、その div 内にバッジ span が含まれる場合、  
`extractInlineBadgeShapes` が親（見出し）レベルでバッジを shape として抽出するが、  
`extractTextRuns` の block 再帰でバッジテキストが `skipInlineBadges=false` で処理されるため、  
バッジが **shape として描画 + 親テキストランにも backgroundColor 付き run として二重挿入** される。

**発生条件**：`html: true` + `<h1>` / `<p>` 直下に `display:flex` div があり、その中にバッジ span がある。通常の Marp Markdown では発生しない。

**優先度**：低（`html: true` + 見出し内 flex container というエッジケース）

#### K01-B: `extractListItems` でバッジが shape として抽出されない（低リスク・仕様）

`extractInlineBadgeShapes` は `walkElements` の `h1-6`・`p`・`blockquote`・`header`・`footer` の各ブランチでのみ呼ばれる。  
`extractListItems` の中では呼ばれない。

**影響**：`html: true` で `<li>` 内に `display:flex` + バッジ span を使うと、バッジが独立した shape にならず、テキストランの `backgroundColor` ハイライトとして出力される。  
視覚的には近いが、丸バッジの border-radius が再現されない。

**優先度**：現状は仕様として許容。バッジ形状が重要な場合は別途対応を検討する。

---

### ADR-16: `<li>` 内の `<br>` 欠落 + リスト間画像の位置ずれ

#### 問題

1. **Trailing-space 改行が `<li>` 内で欠落**  
   Markdown の「行末スペース 2 個 + 改行」（`  \n`）は HTML では `<br>` に変換される。  
   `<p>` や `<h1-6>` ではこの `<br>` が正しく PPTX の `breakLine` run に変換されていたが、  
   `<li>` 内では無視されて行区切りが消えていた。

2. **リスト間画像の位置ずれ**  
   Markdown で空行なしにリスト項目・画像・リスト項目を並べると、markdown-it が  
   画像を最初の `<li>` の lazy continuation として解析する。  
   その結果 `<ul>` のバウンディングボックスが画像領域まで広がり、  
   PPTX では 1 つのテキストボックスとして生成されたため、  
   `extractNestedImages` が追記した画像が「Second item」と同じ y 座標に重なっていた。

#### 根本原因

**Bug 1**  
`extractListItems` の `<li>` 子ノード処理ループの `else` 分岐が、  
`<br>` を `extractTextRuns(<br>)` の **root element** として渡していた。  
`<br>` には子ノードがないため `extractTextRuns` は `[]` を返し、  
`breakLine` ランが生成されなかった。

**Bug 2**  
`walkElements` の `<ul>/<ol>` 分岐は `<ul>` 全体を 1 つのテキストボックスとして出力し、  
その後 `extractNestedImages` で画像を抽出・追記していた。  
しかし `<ul>` の高さが画像領域を含むため、追記した画像の y 座標が  
テキストボックスと重なる位置になっていた。

#### 修正

**Fix 1 (`extractListItemEl` ヘルパー抽出)**  
`extractListItems` の `<li>` 処理ループを `extractListItemEl(li, level)` として分離。  
`<br>` の明示分岐を追加：
...
**Fix 2 (`walkElements` での sub-list 分割)**  
`<ul>/<ol>` が非絵文字 `<img>` を含む `<li>` を持つ場合、  
画像を含む `<li>` 境界でリストを分割する "split path" を追加。  
各 sub-list の y/height は実際の `<li>.getBoundingClientRect()` を使用。  
画像は実際のレンダリング座標で sub-list の合間に inline 挿入される。

#### テスト追加

- `dom-walker.test.ts`：3 件追加
  - `extractListItems — <br> inside tight list <li>`
  - `image embedded between list items (via extractSlides)` × 2
- `slide-builder.test.ts`：2 件追加（計 191 件）
  - `<br> による継続行は invisible bullet で indent が揃えられる`
  - `継続行が複数ある場合もすべて invisible bullet で揃えられる`
- Fixture：`pptx-export.md` に Slide 62（trailing-space 改行）・Slide 63（リスト間画像）を追加

#### ADR-16 補足：`toListTextProps` の indent 修正

dom-walker.ts の Fix 1 で `breakLine: true` ランが正しく生成されたが、  
`slide-builder.ts` の `toListTextProps` が `breakLine: true` を `<a:br/>` として  
出力していると誤って想定していたことで、indent がバレット位置（`marL=0`）にずれていた。

**実際の PptxGenJS 挙動（ADR-16 調査で判明）**

| 方法 | OOXML 出力 |
|---|---|
| `breakLine: true` on a run | 現在の段落を終了し、**新しい `<a:p>` を開始** |
| `bullet` が `truthy` の次ランがある + `opts.align` 未設定 | `else if (bullet)` 分岐で新しい `<a:p>` を開始 |
| `bullet` が `truthy` の次ランがある + `opts.align` 設定済み | **`else if (bullet)` には届かない** — align の `if` がヒットするが align 変化なしで段落は分割されない |

`list` の `addText` 呼び出しには常に `align: el.style.textAlign` が設定されているため、  
`bullet` による新段落トリガーは機能しない。  
継続行も `breakLine: true` で段落を終了させる必要がある。

**Fix 3 (`toListTextProps` の `<br>` 継続行 indent 修正)**  
`item.runs` を `breakLine` ランで分割してグループ化。  
各グループの最後のランには `breakLine: true` を付与して段落を閉じる。  
継続グループ（g > 0）の先頭ランには `bullet: { characterCode: '200B' }`  
（U+200B ゼロ幅スペース＝不可視バレット）を設定し、  
PptxGenJS に `marL = bulletMarL × (1 + level)` のバレット段落を生成させる。  
これにより継続行のテキスト開始位置が先頭行と揃い、  
PowerPoint の Shift+Enter（ソフトリターン）相当の動作を実現する。
